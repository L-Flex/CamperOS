...
static void zb_on_off_state_handler(const event_t *evt, void *arg)
{
    (void)arg;
    if (evt == NULL || !s_joined || s_profile_mgr == NULL) {
        return;
    }

    profile_id_t active = profile_mgr_get_active_id(s_profile_mgr);
    bool on = false;
    uint8_t endpoint = 0;

    if (active == PROFILE_ID_RELAY &&
        (evt->type == EVT_RELAY_ON || evt->type == EVT_RELAY_OFF)) {
        on = (evt->type == EVT_RELAY_ON);
        endpoint = profile_relay_get_endpoint();
    } else if (active == PROFILE_ID_PUMP &&
               (evt->type == EVT_PUMP_ON || evt->type == EVT_PUMP_OFF)) {
        on = (evt->type == EVT_PUMP_ON);
        endpoint = profile_pump_get_endpoint();
    } else {
        return;
    }

    zigbee_device_report_on_off(endpoint, on);
}
...
static esp_err_t zb_add_pump_endpoint(esp_zb_ep_list_t *ep_list)
{
    esp_zb_on_off_light_cfg_t pump_cfg = ESP_ZB_DEFAULT_ON_OFF_LIGHT_CONFIG();
    esp_zb_cluster_list_t *clusters = esp_zb_on_off_light_clusters_create(&pump_cfg);
    if (clusters == NULL) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "registered pump HA endpoint %u (on/off output)",
                 profile_pump_get_endpoint());
    }
    return err;
}
...
static void zb_register_endpoints(profile_mgr_t *profile_mgr)
{
    ...
    } else if (profile_mgr != NULL &&
               profile_mgr_get_active_id(profile_mgr) == PROFILE_ID_PUMP) {
        esp_err_t err = zb_add_pump_endpoint(ep_list);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "pump endpoint registration failed: %s", esp_err_to_name(err));
        }
    }
...
esp_err_t zigbee_device_start(const zigbee_device_deps_t *deps)
{
    ...
    if (deps->event_bus != NULL) {
        event_bus_subscribe(deps->event_bus, EVT_RELAY_ON, zb_on_off_state_handler, NULL);
        event_bus_subscribe(deps->event_bus, EVT_RELAY_OFF, zb_on_off_state_handler, NULL);
        event_bus_subscribe(deps->event_bus, EVT_PUMP_ON, zb_on_off_state_handler, NULL);
        event_bus_subscribe(deps->event_bus, EVT_PUMP_OFF, zb_on_off_state_handler, NULL);
    }
